  ...
  return (
    <Card className="transition-colors hover:bg-muted/50">
      <CardHeader className="flex-row items-start justify-between gap-2 space-y-0">
        <div onClick={() => !editing && router.push(`/resume/${resume._id}`)} className="flex flex-1 items-center gap-1.5 overflow-hidden pt-0.5">
            ...
            <>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    <AlertDialogCancel className={cn(buttonVariants({ variant: "outline", size: "sm" }))}>Cancel</AlertDialogCancel>
                    <AlertDialogAction
      ...
      <CardContent onClick={() => !editing && router.push(`/resume/${resume._id}`)} className="cursor-pointer">
        <p className="text-xs text-muted-foreground">Last updated {resume.updatedAt ? new Date(resume.updatedAt).toLocaleDateString() : "—"}</p>
      </CardContent>
    </Card>
  );
}
